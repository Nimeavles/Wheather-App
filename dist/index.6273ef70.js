...
var DEFAULT_TIMEOUT = 180000 // 3 minutes
;
//
// request
//
function request(options, callback) {
    ...
    if (options.qs) {
        ...
        if (options.uri.indexOf('?') !== -1) options.uri = options.uri + '&' + qs;
        else options.uri = options.uri + '?' + qs;
    }
    //END QS Hack
    //BEGIN FORM Hack
    var multipart = function(obj) {
        ...
        var lines = [];
        for(var p in obj)if (obj.hasOwnProperty(p)) lines.push('--' + result.boundry + "\n" + 'Content-Disposition: form-data; name="' + p + '"' + "\n" + "\n" + obj[p] + "\n");
...
request.defaults = function(options, requester) {
    var def = function(method) {
        var d = function(params, callback) {
            ...
            for(var i in options)if (params[i] === undefined) params[i] = options[i];
...
//# sourceMappingURL=index.6273ef70.js.map
